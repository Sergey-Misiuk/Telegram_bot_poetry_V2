...
async def fetch_html(url):
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as client:
            ...
            response.raise_for_status()  # Бросит исключение, если код ответа не 200-299
...
async def parse_poem(url):
    ...
    try:
        ...
        # Возвращаем объект Poem с очищенной информацией
        return Poem(author=author, title=title, text=poem_text)

    except AttributeError:
        print(f"Ошибка парсинга данных на {url}")
        return None
